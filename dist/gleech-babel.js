...
export default (() => ({
  blueShift(factor, cb) {
    if (!nullOrUndefined(factor)) {
      if ("number" != typeof factor) return throwError.call(this, "factor must be a number", cb);
      if (factor < 2) return throwError.call(this, "factor must be greater than 1", cb);
    }
...
Jimp.prototype.colorShift2 = function colorShift2(dir, cb) {
	if (!nullOrUndefined(dir)) return throwError.call(this, "dir must be truthy or falsey", cb);
	var width = this.bitmap.width,
	    height = this.bitmap.height,
	    data = this.bitmap.data;
...
Jimp.prototype.ditherBayer = function ditherBayer(map, cb) {
	...
	if ("number" !== typeof map) return throwError.call(this, "map must be a number", cb);
	if (map < 0 || map > 2) return throwError.call(this, "map must be a number from 0 to 2", cb);
	...
	threshold_maps = [[[3, 7, 4], [6, 1, 9], [2, 8, 5]], [[1, 9, 3, 11], [13, 5, 15, 7], [4, 12, 2, 10], [16, 8, 14, 6]], [[1, 49, 13, 61, 4, 52, 16, 64], [33, 17, 45, 29, 36, 20, 48, 32], [9, 57, 5, 53, 12, 60, 8, 56], [41, 25, 37, 21, 44, 28, 40, 24], [3, 51, 15, 63, 2, 50, 14, 62], [35, 19, 47, 31, 34, 18, 46, 30], [11, 59, 7, 55, 10, 58, 6, 54], [43, 27, 39, 23, 42, 26, 38, 22]]],
...
Jimp.prototype.ditherBayer3 = function ditherBayer3(map, cb) {
	map = !nullOrUndefined(map) ? map : randFloor(3);
	if (typeof map !== 'number') return throwError.call(this, "map must be a number", cb);
	if (map < 0 || map > 2) return throwError.call(this, "map must be a number from 0 to 2", cb);
	var width = this.bitmap.width,
	    height = this.bitmap.height,
	    data = this.bitmap.data,
...
Jimp.prototype.ditherBitmask = function ditherBitmask(mask, cb) {
	if (!nullOrUndefined(mask)) {
		if ("number" != typeof mask) return throwError.call(this, "mask must be a number", cb);
		if (mask < 0 || mask > 254) return throwError.call(this, "mask must be a number from 0 to 2", cb);
	}
	var width = this.bitmap.width,
	    height = this.bitmap.height,
	    data = this.bitmap.data,
...
Jimp.prototype.focusImage = function focusImage(pixelation, cb) {
	if (!nullOrUndefined(pixelation)) {
		if ("number" != typeof pixelation) return throwError.call(this, "pixelation must be a number", cb);
		if (pixelation < 2) return throwError.call(this, "pixelation must be greater than 1", cb);
	}
	var width = this.bitmap.width,
	    height = this.bitmap.height,
...
Jimp.prototype.fractal = function fractal(type, cb) {
	if (!nullOrUndefined(type)) {
		if (typeof type != 'number') return throwError.call(this, "type must be a number", cb);
		if (type < 0 || type > 1) return throwError.call(this, "type must be a 0 or 1", cb);
	}
	var width = this.bitmap.width,
	    height = this.bitmap.height,
	    data = this.bitmap.data;
...
Jimp.prototype.fractalGhosts = function fractalGhosts(type, color, cb) {
	if (!nullOrUndefined(type)) {
		if (typeof type != 'number') return throwError.call(this, "type must be a number", cb);
		if (type < 0 || type > 3) return throwError.call(this, "type must be a between 0 and 3", cb);
	}
	if (!nullOrUndefined(color)) {
		if (typeof color != 'number') return throwError.call(this, "color must be a number", cb);
		if (color < 0 || color > 4) return throwError.call(this, "color must be a between 0 and 4", cb);
	}
	var width = this.bitmap.width,
	    height = this.bitmap.height,
	    data = this.bitmap.data,
...
Jimp.prototype.randomGlitch = function (cb) {
	...
	for (var prop in Jimp.prototype) {
		if (typeof Jimp.prototype[prop] === 'function' && Jimp.prototype[prop].name) {
...
Jimp.prototype.selectSlice = function selectSlice(selection, cb) {
	if (!nullOrUndefined(selection)) {
		if ("number" != typeof selection) return throwError.call(this, "selection must be a number", cb);
		if (selection < 0 && selection > 1) return throwError.call(this, "selection must be 0 or 1", cb);
	}
	var width = this.bitmap.width,
	    height = this.bitmap.height,
	    data = this.bitmap.data,
...
Jimp.prototype.slice = function slice(cutstart, cutend, cb) {
	if (!nullOrUndefined(cutstart)) {
		if ("number" != typeof cutstart) return throwError.call(this, "cutstart must be a number", cb);
		if (cutstart > 0 && cutstart < cutend) return throwError.call(this, "cutstart must be greater than 0 and less than cutend", cb);
	}
	if (!nullOrUndefined(cutend)) {
		if ("number" != typeof cutend) return throwError.call(this, "cutend must be a number", cb);
		if (cutend > 0 && cutend > cutstart) return throwError.call(this, "cutend must be greater than 0 and greater than cutstart", cb);
	}
	var width = this.bitmap.width,
	    height = this.bitmap.height,
	    data = this.bitmap.data;
...
Jimp.prototype.superShift = function superShift(iter, dir, cb) {
	if (!nullOrUndefined(iter)) {
		if ("number" != typeof iter) return throwError.call(this, "iter must be a number", cb);
		if (iter < 2) return throwError.call(this, "iter must be greater than 1", cb);
	}
	if (!nullOrUndefined(dir)) return throwError.call(this, "dir must be truthy or falsey", cb);
	var width = this.bitmap.width,
	    height = this.bitmap.height,
...
Jimp.prototype.superSlice = function superSlice(iter, cb) {
	if (!nullOrUndefined(iter)) {
		if ("number" != typeof iter) return throwError.call(this, "iter must be a number", cb);
		if (iter > 0) return throwError.call(this, "iter must be greater than 0", cb);
	}
	var width = this.bitmap.width,
	    height = this.bitmap.height,
	    data = this.bitmap.data,
...
function throwError(err, cb) {
	...
	if (typeof cb === 'function') return cb.call(this, err);else throw err;
}
...
function randSort(a, b) {
	var sort = randChoice([coinToss, leftSort, rightSort, redSort, greenSort, blueSort, avgSort]);
	return sort(a, b);
}
